# Design notes (pre-calibration)

This is the README as it stood before the bench session of 20 Aug 2026, when
the repo was refocused onto calibration. It is kept because it carries design
rationale that exists nowhere else -- the anti-backlash correction policy, why
zero sits at mid-stroke, why a saved position is checked rather than trusted.

Treat the status claims in it as historical. The live status is in the
top-level README.
